...
import { savePost } from '@/utils/blogStore';
...
export default function NewPostPage() {
  ...
  const updateBlock = (index: number, value: string, key: string = 'value') => {
    const newContent = [...post.content] as Record<string, unknown>[];
    newContent[index][key] = value;
    setPost(prev => ({ ...prev, content: newContent as ContentBlock[] }));
  };
    ...
    <div className="w-full min-h-screen bg-neutral-950 pt-32 pb-32">
      <div className="max-w-4xl mx-auto px-6">
        <div className="fixed top-0 left-0 w-full bg-neutral-950/80 backdrop-blur-md border-b border-neutral-900 z-50 py-4 px-6 flex justify-between items-center">
          <button onClick={() => router.push('/admin')} className="text-neutral-500 hover:text-white flex items-center gap-2 text-xs uppercase font-bold tracking-widest">
          ...
          {post.content.map((block, idx) => (
            <motion.div key={idx} layout className="relative group pl-12">
              <div className="absolute left-0 top-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button onClick={() => removeBlock(idx)} className="text-neutral-600 hover:text-red-500 p-2">
              ...
              {block.type === 'image' && (
                <div className="bg-neutral-900 p-4 rounded-sm border border-neutral-800">
                  ...
                  {block.src && (
                    ...
                    <img src={block.src} alt="" className="w-full max-h-[400px] object-cover rounded-sm" />
                  )}
                  <input
                    type="text"
                    value={block.caption || ''}
