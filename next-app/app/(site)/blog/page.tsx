        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-12 gap-y-20">
          {posts.map((post, idx) => (
            ...
            >
              <Link href={`/blog/${post.id}`} className="block flex-grow">
                <div className="w-full aspect-[4/3] bg-neutral-900 mb-8 overflow-hidden relative">
                  ...
                  <img
                    src={post.image}
                    alt={post.title}
                    className="w-full h-full object-cover grayscale group-hover:scale-105 transition-transform duration-1000 opacity-80 group-hover:opacity-100"
                  />
                  {post.layout === 'magazine' && (
                    <div className="absolute top-4 right-4 bg-white text-black text-[10px] font-bold uppercase tracking-widest px-2 py-1">Editorial</div>
                  )}
                </div>
              ...
              <Link href={`/blog/${post.id}`} className="flex items-center text-xs font-bold uppercase tracking-widest text-white group-hover:text-neutral-400 transition-colors mt-auto">
