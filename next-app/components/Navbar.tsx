      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center gap-12">
          {navLinks.map((link) => (
            ...
            >
              {link.name}
              <span className={`absolute -bottom-1 left-0 w-full h-[1px] bg-white origin-left transform scale-x-0 transition-transform duration-300 ease-out group-hover:scale-x-100 ${pathname === link.path ? 'scale-x-100' : ''}`}></span>
            </Link>
          ))}
          <button
